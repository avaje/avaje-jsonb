...
class BufferRecycleTracker {
   ...
   */
  static BufferRecycleTracker instance() {
  ...
  int releaseBuffers() {
    ...
    try {
      ...
      for (SoftReference<BufferRecycler> ref : tracked.keySet()) {
        ref.clear(); // possibly already cleared by gc, nothing happens in that case
        ++count;
      }
      tracked.clear(); //release cleared SoftRefs
      return count;
    } finally {
      lock.unlock();
    }
  ...
  SoftReference<BufferRecycler> wrapAndTrack(BufferRecycler br) {
    SoftReference<BufferRecycler> newRef = new SoftReference<>(br, refQueue);
    // also retain softRef to br in a set to be able to release it on shutdown
    tracked.put(newRef, true);
  ...
  private void removeSoftRefsClearedByGc() {
    ...
    while ((clearedSoftRef = (SoftReference<?>) refQueue.poll()) != null) {
      // uses reference-equality, quick, and O(1) removal by HashSet
      tracked.remove(clearedSoftRef);
    }
  }
