# Avaje Jsonb Guides

Step-by-step guides for avaje-jsonb JSON library.

## Getting Started

- [Basic Usage](basic-usage.md) - Serialize and deserialize JSON
- [Custom Adapters](custom-adapters.md) - Create custom JSON adapters
- [Property Mapping](property-mapping.md) - Map JSON properties to Java fields

## Advanced Topics

- [Streaming](streaming.md) - Stream large JSON efficiently
- [Polymorphic Types](polymorphic-types.md) - Handle polymorphic JSON
- [File Chaining](file-chaining.md) - Chain JSON operations
- [Testing](testing.md) - Test JSON serialization
- [Configuration & Tuning](configuration.md) - Buffer sizes, parser limits, ENV vars
